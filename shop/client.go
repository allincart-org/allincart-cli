...
func newShopCredentials(config *Config) (adminSdk.OAuthCredentials, error) {
	clientId, clientSecret := os.Getenv("ALLINCART_CLI_API_CLIENT_ID"), os.Getenv("ALLINCART_CLI_API_CLIENT_SECRET")

	if clientId != "" && clientSecret != "" {
		return adminSdk.NewIntegrationCredentials(clientId, clientSecret, []string{"write"}), nil
	}

	username, password := os.Getenv("ALLINCART_CLI_API_USERNAME"), os.Getenv("ALLINCART_CLI_API_PASSWORD")
...
func NewShopClient(ctx context.Context, config *Config) (*adminSdk.Client, error) {
	...
	if os.Getenv("ALLINCART_CLI_API_DISABLE_SSL_CHECK") == "true" {
		skipSSLCert = true
	}
	...
	shopUrl := os.Getenv("ALLINCART_CLI_API_URL")
