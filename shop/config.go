...
type Config struct {
	...
	// The URL of the Allincart instance
	URL              string            `yaml:"url"`
...
type ConfigBuild struct {
	...
	// When enabled, the assets of extensions will be removed from the extension public folder. (Requires Allincart 6.5.2.0)
...
func ReadConfig(fileName string, allowFallback bool) (*Config, error) {
	...
	if os.IsNotExist(err) {
		...
		return nil, fmt.Errorf("cannot find project configuration file \"%s\", use allincart-cli project config init to create one", fileName)
	}
...
func DefaultConfigFileName() string {
	...
	if err != nil {
		return ".allincart-project.yml"
	}

	if _, err := os.Stat(path.Join(currentDir, ".allincart-project.yaml")); err == nil {
		return ".allincart-project.yaml"
	}
