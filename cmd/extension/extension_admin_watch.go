...
var (
	...
	extensionEsbuildRegExp = regexp.MustCompile(`(?m)/.allincart-cli/([a-z0-9-]+)/(.*)$`)
)
...
var extensionAdminWatchCmd = &cobra.Command{
	...
	Short: "Extremely fast ESBuild powered Allincart 6 Administration watcher",
			...
			if req.URL.Path == targetShopUrl.Path+"/api/_info/config" {
				...
				for _, ext := range esbuildInstances {
					bundleInfo.Bundles[ext.name] = adminBundlesInfoAsset{
						Css:        []string{fmt.Sprintf("%s/.allincart-cli/%s/extension.css", browserUrl.String(), ext.assetName)},
						Js:         []string{fmt.Sprintf("%s/.allincart-cli/%s/extension.js", browserUrl.String(), ext.assetName)},
						LiveReload: true,
						Name:       ext.assetName,
					}
				}

				bundleInfo.Bundles["AllincartCLI"] = adminBundlesInfoAsset{Css: []string{}, Js: []string{browserUrl.String() + "/__internal-admin-proxy/live-reload.js"}}
