...
import (
	...
	"github.com/allincart-org/allincart-cli/internal/llm"
	"github.com/allincart-org/allincart-cli/internal/twigparser"
	"github.com/allincart-org/allincart-cli/internal/verifier"
...
var extensionAiTwigUpgradeCmd = &cobra.Command{
	Use:   "twig-upgrade [path] [old-allincart-version] [new-allincart-version]",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, sourceDirectory := range toolCfg.SourceDirectories {
			...
			oldVersion, err := cloneAllincartStorefront(cmd.Context(), args[1])
			if err != nil {
				return err
			}
			...
			newVersion, err := cloneAllincartStorefront(cmd.Context(), args[2])
			if err != nil {
				return err
			}
...
func cloneAllincartStorefront(ctx context.Context, version string) (string, error) {
	tempDir, err := os.MkdirTemp(os.TempDir(), "allincart")
	if err != nil {
		return "", err
	}

	git := exec.CommandContext(ctx, "git", "-c", "advice.detachedHead=false", "clone", "-q", "--branch", "v"+version, "https://github.com/allincart/storefront", tempDir, "--depth", "1")
