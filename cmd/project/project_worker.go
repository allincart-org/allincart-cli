...
var projectWorkerCmd = &cobra.Command{
	...
	RunE: func(cobraCmd *cobra.Command, args []string) error {
		...
		if projectRoot, err = findClosestAllincartProject(); err != nil {
			return err
		}
		...
		if queuesToConsume == "" {
			if is, _ := shop.IsAllincartVersion(projectRoot, ">=6.5.7"); is {
				consumeArgs = append(consumeArgs, "async", "failed", "low_priority")
			} else if is, _ := shop.IsAllincartVersion(projectRoot, ">=6.5"); is {
		...
		baseName := fmt.Sprintf("allincart-cli-%d", os.Getpid())
