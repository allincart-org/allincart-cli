...
var projectImageProxyCmd = &cobra.Command{
	...
	Long: `Start an HTTP server that serves files from the public folder of the closest Allincart project.
If a file is not found locally, it proxies the request to the upstream server.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		path, err := findClosestAllincartProject()
		if err != nil {
			return err
		}
		...
		if upstreamURL == "" {
			return fmt.Errorf("upstream URL must be provided either via --url flag or in .allincart-project.yml")
		}
		...
		if !imageProxySkipConfig {
			...
			// Determine the URL to use in Allincart config
			...
			// Write Allincart configuration
			configContent := fmt.Sprintf(`allincart:
  filesystem:
    public:
      type: "local"
      url: '%s'
...
`, configURL)
			...
			if err := os.WriteFile(configFile, []byte(configContent), 0644); err != nil {
				return fmt.Errorf("failed to write Allincart config: %w", err)
			}

			logging.FromContext(cmd.Context()).Infof("Created Allincart config: %s (URL: %s)", configFile, configURL)
			...
			cleanup = func() {
				if err := os.Remove(configFile); err != nil && !os.IsNotExist(err) {
					...
					logging.FromContext(cmd.Context()).Infof("Removed Allincart config: %s", configFile)
				}
			}
			...
			logging.FromContext(cmd.Context()).Infof("Skipping Allincart config file creation")
		}
...
func init() {
	...
	projectImageProxyCmd.Flags().StringVar(&imageProxyExternalURL, "external-url", "", "External URL for Allincart config (e.g., for reverse proxy setups)")
	projectImageProxyCmd.Flags().BoolVar(&imageProxySkipConfig, "skip-config", false, "Skip creating Allincart config file")
}
