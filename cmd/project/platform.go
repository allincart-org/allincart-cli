...
func findClosestAllincartProject() (string, error) {
	...
	for {
		...
		for _, file := range files {
			if _, err := os.Stat(file); err == nil {
				...
				if strings.Contains(contentString, "allincart/core") {
	...
	return "", fmt.Errorf("cannot find Allincart project in current directory")
}
