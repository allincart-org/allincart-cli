...
func (MailTemplateSync) Pull(ctx adminSdk.ApiContext, client *adminSdk.Client, config *shop.Config) error {
	...
	for _, row := range mailTemplates.Data {
		...
		for _, translation := range row.Translations {
			...
			htmLFilePath := fmt.Sprintf(".allincart-cli/mail-template/%s/%s-html.twig", row.MailTemplateType.TechnicalName, configKey)
			plainFilePath := fmt.Sprintf(".allincart-cli/mail-template/%s/%s-plain.twig", row.MailTemplateType.TechnicalName, configKey)

			if ok := duplicateTypes[row.MailTemplateType.TechnicalName]; ok {
				htmLFilePath = fmt.Sprintf(".allincart-cli/mail-template/%s/%s-%s-html.twig", row.MailTemplateType.TechnicalName, configKey, row.Id)
				plainFilePath = fmt.Sprintf(".allincart-cli/mail-template/%s/%s-%s-plain.twig", row.MailTemplateType.TechnicalName, configKey, row.Id)
			}
