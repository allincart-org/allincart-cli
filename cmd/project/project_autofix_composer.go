...
var projectAutofixComposerCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := huh.NewInput().
			Title("Please enter the Allincart Packagist Token").
			Value(&token).
			Run(); err != nil {
			return err
		}
		...
		for _, extension := range extensions {
			...
			composerInstall = append(composerInstall, fmt.Sprintf("store.allincart.net/%s:%s", strings.ToLower(extName), extVersion.String()))
		...
		if !rootComposerJson.Repositories.HasRepository("https://packages.allincart.net") {
			rootComposerJson.Repositories = append(rootComposerJson.Repositories, packagist.ComposerJsonRepository{
				Type: "composer",
				URL:  "https://packages.allincart.net",
			})
		}
		...
		auth.BearerAuth["packages.allincart.net"] = token
