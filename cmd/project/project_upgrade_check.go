...
var projectUpgradeCheckCmd = &cobra.Command{
	Use:   "upgrade-check",
	Short: "Check that installed extensions are compatible with a future Allincart version",
	RunE: func(cmd *cobra.Command, args []string) error {
		project, err := findClosestAllincartProject()
		if err != nil {
			return err
		}
		...
		corePackage := composerLock.GetPackage("allincart/core")

		if corePackage == nil {
			return fmt.Errorf("allincart/core package not found in composer.lock")
		}
		...
		versions, err := extension.GetAllincartVersions(cmd.Context())
		if err != nil {
			return err
		}
		...
		if len(possibleVersions) == 0 {
			fmt.Println("You are on the latest version of Allincart")
			return nil
		}
		...
		prompt := huh.NewSelect[string]().
			Height(10).
			Title("Select a Allincart version to check compatibility").
			Options(
