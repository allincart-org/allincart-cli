...
func TestManifestRead(t *testing.T) {
	...
	assert.Equal(t, "https://instorier.apps.allincart.io/app/lifecycle/register", manifest.Setup.RegistrationUrl)
	assert.Equal(t, "", manifest.Setup.Secret)

	assert.Equal(t, "https://instorier.apps.allincart.io/iframe", manifest.Admin.BaseAppUrl)
