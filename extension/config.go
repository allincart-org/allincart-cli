...
type ConfigBuild struct {
	...
	// Override the allincart version constraint for building, can be used to specify the version of the allincart to use for building
	AllincartVersionConstraint string `yaml:"allincartVersionConstraint,omitempty"`
...
type ConfigBuildZipAssets struct {
	// When enabled, the allincart-cli build the assets
	Enabled bool `yaml:"enabled"`
...
type ConfigStore struct {
	...
	// Specifies whether the extension should automatically be set compatible with Allincart bugfix versions.
...
func readExtensionConfig(dir string) (*Config, error) {
	...
	config.FileName = ".allincart-extension.yml"

	configLocation := ""

	if _, err := os.Stat(filepath.Join(dir, ".allincart-extension.yml")); err == nil {
		configLocation = filepath.Join(dir, ".allincart-extension.yml")
	} else if _, err := os.Stat(filepath.Join(dir, ".allincart-extension.yaml")); err == nil {
		configLocation = filepath.Join(dir, ".allincart-extension.yaml")
	} else {
		return config, nil
	}
