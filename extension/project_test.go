...
func TestGetAllincartProjectConstraintComposerJson(t *testing.T) {
	testCases := []struct {
		Name       string
...
final public const ALLINCART_FALLBACK_VERSION = '6.6.9999999.9999999-dev';
`,
			},
			Constraint: "6.5.*",
		},
		{
			Name: "Get constraint from kernel (allincart/allincart case)",
			Files: map[string]string{
				"composer.json":          `{}`,
				"src/Core/composer.json": `{}`,
				"src/Core/Kernel.php": `<?php
...
`,
		...
		{
			Name: "composer.json with no allincart package",
			Files: map[string]string{
				"composer.json": `{}`,
			},
			Error: "missing allincart/core requirement in composer.json",
		},
		...
		{
			Name: "composer.json malformed version, lock does not contain allincart/core",
			Files: map[string]string{
				"composer.json": `{
		"require": {
			"allincart/core": "6.5.*"
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			constraint, err := GetAllincartProjectConstraint(tmpDir)
