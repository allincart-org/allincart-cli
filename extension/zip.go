...
import (
	...
	"github.com/allincart-org/allincart-cli/internal/changelog"
...
func PrepareFolderForZipping(ctx context.Context, path string, ext Extension, extCfg *Config) error {
	...
	minAllincartVersionConstraint, err := ext.GetAllincartVersionConstraint()
	if err != nil {
		return fmt.Errorf(errorFormat, err)
	}

	minVersion, err := lookupForMinMatchingVersion(ctx, minAllincartVersionConstraint)
	if err != nil {
		return fmt.Errorf("lookup for min matching version: %w", err)
	}

	allincart65Constraint, _ := version.NewConstraint(">=6.5.0")

	if allincart65Constraint.Check(version.Must(version.NewVersion(minVersion))) {
		logging.FromContext(ctx).Info("Allincart 6.5 detected, disabling composer replacements")
		return nil
	}
...
func filterRequires(composer map[string]interface{}, extCfg *Config) map[string]interface{} {
	...
	keys := []string{"allincart/platform", "allincart/core", "allincart/allincart", "allincart/storefront", "allincart/administration", "allincart/elasticsearch", "composer/installers"}
	if extCfg != nil {
...
func addComposerReplacements(composer map[string]interface{}, minVersion string) (map[string]interface{}, error) {
	...
	for _, component := range components {
		packageName := fmt.Sprintf("allincart/%s", component)
...
func GetAllincartVersions(ctx context.Context) ([]string, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://repo.packagist.org/p2/allincart/core.json", http.NoBody)
	if err != nil {
		return nil, fmt.Errorf("create composer version request: %w", err)
	}

	req.Header.Set("User-Agent", "Allincart CLI")
...
func lookupForMinMatchingVersion(ctx context.Context, versionConstraint *version.Constraints) (string, error) {
	versions, err := GetAllincartVersions(ctx)
	if err != nil {
		return "", fmt.Errorf("get allincart versions: %w", err)
	}
