...
import (
	...
	"github.com/allincart-org/allincart-cli/internal/esbuild"
...
type AssetBuildConfig struct {
	...
	AllincartRoot                string
	AllincartVersion             *version.Constraints
	Browserslist                 string
...
func BuildAssetsForExtensions(ctx context.Context, sources []asset.Source, assetConfig AssetBuildConfig) error { // nolint:gocyclo
	...
	minVersion, err := lookupForMinMatchingVersion(ctx, assetConfig.AllincartVersion)
	if err != nil {
		return err
	}

	requiresAllincartSources := cfgs.RequiresAllincartRepository()

	allincartRoot := assetConfig.AllincartRoot
	if allincartRoot == "" && requiresAllincartSources {
		allincartRoot, err = setupAllincartInTemp(ctx, minVersion)
		if err != nil {
			return err
		}
	...
	if !assetConfig.DisableAdminBuild && cfgs.RequiresAdminBuild() {
		...
		if len(nonCompatibleExtensions) != 0 {
			if err := prepareAllincartForAsset(allincartRoot, nonCompatibleExtensions); err != nil {
				return err
			}
...
func prepareAllincartForAsset(allincartRoot string, cfgs map[string]ExtensionAssetConfigEntry) error {
	varFolder := fmt.Sprintf("%s/var", allincartRoot)
	if _, err := os.Stat(varFolder); os.IsNotExist(err) {
		err := os.Mkdir(varFolder, os.ModePerm)
		if err != nil {
			return fmt.Errorf("prepareAllincartForAsset: %w", err)
		}
	}
...
func setupAllincartInTemp(ctx context.Context, minVersion string) (string, error) {
...
func (c ExtensionAssetConfig) RequiresAllincartRepository() bool {
	for _, entry := range c {
