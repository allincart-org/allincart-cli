...
import (
	...
	"github.com/allincart-org/allincart-cli/internal/phplint"
...
type platformComposerJsonExtra struct {
	AllincartPluginClass string            `json:"allincart-plugin-class"`
...
func (p PlatformPlugin) GetName() (string, error) {
	if p.Composer.Extra.AllincartPluginClass == "" {
		return "", fmt.Errorf("extension name is empty")
	}

	parts := strings.Split(p.Composer.Extra.AllincartPluginClass, "\\")
...
func (p PlatformPlugin) GetAllincartVersionConstraint() (*version.Constraints, error) {
...
func (p PlatformPlugin) Validate(c context.Context, ctx *ValidationContext) {
	...
	if p.Composer.Type == "" {
		...
		ctx.AddError("metadata.type", "The composer type must be allincart-platform-plugin")
	}
	...
	if len(p.Composer.Require) == 0 {
		...
		_, exists := p.Composer.Require["allincart/core"]

		if !exists {
			ctx.AddError("metadata.require", "You need to require \"allincart/core\" package")
		}
	}
...
func validatePHPFiles(c context.Context, ctx *ValidationContext) {
	constraint, err := ctx.Extension.GetAllincartVersionConstraint()
	if err != nil {
		ctx.AddError("php.linter", fmt.Sprintf("Could not parse allincart version constraint: %s", err.Error()))
		return
	}
...
func GetPhpVersion(ctx context.Context, constraint *version.Constraints) (string, error) {
	...
	var allincartToPHPVersion map[string]string

	err = json.NewDecoder(resp.Body).Decode(&allincartToPHPVersion)
	if err != nil {
		return "", err
	}

	for allincartVersion, phpVersion := range allincartToPHPVersion {
		allincartVersionConstraint, err := version.NewVersion(allincartVersion)
		if err != nil {
			continue
		}

		if constraint.Check(allincartVersionConstraint) {
			return phpVersion, nil
		}
	}

	return "", errors.New("could not find php version for allincart version")
}
