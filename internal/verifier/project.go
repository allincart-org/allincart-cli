...
func getAllincartConstraint(root string) (*version.Constraints, error) {
	composerJson := path.Join(root, "composer.json")
	...
	var composerJsonData struct {
		Require struct {
			Allincart string `json:"allincart/core"`
		} `json:"require"`
	}
	...
	if composerJsonData.Require.Allincart == "" {
		return nil, fmt.Errorf("allincart/core is not required")
	}

	cst, err := version.NewConstraint(composerJsonData.Require.Allincart)
	if err != nil {
		return nil, err
	}
...
func GetConfigFromProject(root string) (*ToolConfig, error) {
	constraint, err := getAllincartConstraint(root)
	if err != nil {
		return nil, err
	}
	...
	shopCfg, err := shop.ReadConfig(path.Join(root, ".allincart-project.yml"), true)
	if err != nil {
		return nil, err
	}
