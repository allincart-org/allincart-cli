export default {
  meta: {
    ...
    docs: {
      description:
        "Replace Allincart.State with Allincart.Store (and destructured State accordingly).",
      category: "Best Practices",
      recommended: false
    },
    fixable: "code",
    schema: [],
    minAllincartVersion: '6.7.0.0'
  },
  ...
  create(context) {
    ...
    function isAllincartState(node) {
      return (
        node &&
    ...
    return {
      // Check for destructuring: const { State } = Allincart;
      VariableDeclarator(node) {
        if (
          node.init &&
          node.init.type === "Identifier" &&
          node.init.name === "Allincart" &&
            ...
            computedText = props
              .map((prop) => {
                return (
                  ...
                  baseIndent +
                  "    " +
                  `return Allincart.Store.get('${storeName}').${prop};\n` +
                  baseIndent +
              ...
              .map((prop) => {
                ...
                if (
                  ...
                  return (
                    ...
                    baseIndent +
                    "    " +
                    `return Allincart.Store.get('${storeName}').${mappedProp};\n` +
                    baseIndent +
                    "},"
                  );
                }
      ...
      CallExpression(node) {
        if (
          ...
          if (
            ...
            stateMember.object.name === "Allincart" &&
          ...
          if (
            ...
            if (parts.length === 2) {
              ...
              const argsText = args
                ...
                .join(", ");
              const newCode = `${shortHand ? '' : 'Allincart.'}Store.get('${storeName}').${methodName}(${argsText ? argsText : ""
                })`;
              context.report({
                node,
                message:
                  "Replace State.commit/Allincart.State.commit call with Allincart.Store.get call.",
                fix(fixer) {
