...
func SetupTools(ctx context.Context, currentVersion string) error {
	customToolDir := os.Getenv("ALLINCART_CLI_TOOLS_DIR")
	if customToolDir != "" {
	...
	cacheDir := system.GetAllincartCliCacheDir()
	...
	if ok, err := system.IsPHPVersionAtLeast("8.2.0"); err != nil {
		...
		return fmt.Errorf("php version must be at least 8.2.0 to use this. Update your PHP version or use the allincart-cli docker image")
	}

	if ok, err := system.IsNodeVersionAtLeast("22.0.0"); err != nil {
		return fmt.Errorf("failed to check installed Node.js version: %w", err)
	} else if !ok {
		return fmt.Errorf("node.js version must be at least 22.0.0 to use this. Update your Node.js version or use the allincart-cli docker image")
	}
