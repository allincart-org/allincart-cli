...
func TestMigrateComposerJson(t *testing.T) {
	t.Run("successful migration", func(t *testing.T) {
		...
		initialComposer := &packagist.ComposerJson{
			Name: "allincart/project",
			Require: packagist.ComposerPackageLink{
				"allincart/recovery": "1.0.0",
				"php":                "^7.4",
			},
		...
		assert.False(t, migratedComposer.HasPackage("allincart/recovery"))
		...
		assert.Equal(t, "*", migratedComposer.RequireDev["allincart/dev-tools"])
