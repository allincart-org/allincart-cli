...
func TestGetPublicVCSURL(t *testing.T) {
	...
	runCommand(t, tmpDir, "remote", "add", "origin", "https://github.com/FriendsOfAllincart/FroshTools.git")

	url, err = GetPublicVCSURL(t.Context(), tmpDir)
	assert.Equal(t, "https://github.com/FriendsOfAllincart/FroshTools/commit", url)
	assert.NoError(t, err)

	runCommand(t, tmpDir, "remote", "set-url", "origin", "git@github.com:FriendsOfAllincart/FroshTools.git")
