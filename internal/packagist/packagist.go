...
func (p *PackageResponse) HasPackage(name string) bool {
	expectedName := fmt.Sprintf("store.allincart.net/%s", strings.ToLower(name))
...
func GetPackages(ctx context.Context, token string) (*PackageResponse, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://packages.allincart.net/packages.json", nil)
	if err != nil {
		return nil, err
	}
