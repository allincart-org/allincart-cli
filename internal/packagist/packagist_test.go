...
func TestPackageResponseHasPackage(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			responseData: map[string]map[string]PackageVersion{
				"store.allincart.net/swagextensionstore": {
					"1.0.0": {
						Version: "1.0.0",
					},
				},
...
func TestGetPackages(t *testing.T) {
	...
	t.Run("successful request", func(t *testing.T) {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			// Check request
			assert.Equal(t, "Allincart CLI", r.Header.Get("User-Agent"))
		...
		assert.Equal(t, "1.0.0", packages.Packages["store.allincart.net/swagextensionstore"]["1.0.0"].Version)
	})
